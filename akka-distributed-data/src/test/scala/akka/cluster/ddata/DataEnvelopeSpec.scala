...
class DataEnvelopeSpec extends WordSpec with Matchers {
  ...
  "DataEnvelope" must {
    ...
    "handle pruning transitions" in {
      ...
      d4.data.asInstanceOf[GCounter].usingNodes should ===(Set(node2))
    }
    ...
    "merge correctly" in {
      ...
      d3.data.asInstanceOf[GCounter].usingNodes should ===(Set(node1, node2))
      val d4 = d3.initRemovedNodePruning(node1, node2)
      val d5 = d4.prune(node1, PruningPerformed(obsoleteTimeInFuture))
      d5.data.asInstanceOf[GCounter].usingNodes should ===(Set(node2))
      ...
      d6.data.asInstanceOf[GCounter].usingNodes should ===(Set(node2))
